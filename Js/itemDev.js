...
function fillItemTable(page){
	...
	$.ajax({
		...
		$('#pagination').empty();
		$('#pagination').append('<li id="previousArrow" onclick="fillItemTable('+ previousPage +')">'+
		...
		for (var i = 1; i <= response.nbPage; i++) {
			$('#pagination').append('<li id="page'+i+'"><a href="#" onclick="fillItemTable('+i+')">'+i+'</a></li>');
			if (i == response.page) {
				$('#page'+i).attr('class', 'active');
			}
		}

		$('#pagination').append('<li id="nextArrow" onclick="fillItemTable('+nextPage+')">'+
